...
def parse_items(wb):
    if "Item" in wb.sheetnames:
        ws = wb["Item"]
    elif "DB" in wb.sheetnames:
        ws = wb["DB"]
    else:
        raise KeyError(
            "Item master worksheet not found. Expected a sheet named 'Item' or 'DB'. "
    ...
    for row in rows_from_sheet(ws):
        ...
        item = items.setdefault(
            item_no,
            {"item_no": item_no, "ti_format": "standard", "core1": {}, "core2": {}, "core3": {}},
        )
...
def main():
    ...
    wb = openpyxl.load_workbook(
        args.input,
        data_only=True,
        keep_vba=Path(args.input).suffix.lower() == ".xlsm",
    )
    ...
    with output.open("w", encoding="utf-8") as file:
        file.write(
            ...
            "Requires the existing Supabase schema.\n"
        )
        file.write("begin;\n\n")
