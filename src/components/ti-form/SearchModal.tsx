...
import { useDistinctCtTypes, useListTiRecords, type ApprovalStatus } from "@/api-client";
...
export function SearchModal({
  ...
  useEffect(() => {
    if (!open) return;
    setDraftFilters((current) => ({ ...current, approvalStatus: statusFilter }));
    setAppliedFilters((current) => ({ ...current, approvalStatus: statusFilter }));
  }, [open, statusFilter]);
  ...
  const clearSearch = () => {
    setDraftFilters({ ...EMPTY_FILTERS, approvalStatus: statusFilter });
    setAppliedFilters({ ...EMPTY_FILTERS, approvalStatus: statusFilter });
  };
            ...
            <tbody className="divide-y divide-gray-200">
              {records.map((record) => (
                <tr key={record.id} className="odd:bg-white even:bg-gray-50/40 hover:bg-[#eef4ff] transition-colors">
                  <td className="px-3 py-3 font-semibold text-[#2a4080] truncate" title={record.ti_no}>
                    <div className="truncate">{record.ti_no}</div>
                    <div className={`mt-1 text-[10px] uppercase ${statusTone(record.approval_status)}`}>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-1">
                      ...
                      {onEdit && canEditRecords && record.approval_status !== "checked" && (
                        <ActionButton icon={<Pencil />} label="Edit" onClick={() => onEdit(record.ti_no)} tone="green" />
                      )}
                      <ActionButton
                        icon={<Download />}
                        label="PDF"
                        ...
                        tone="purple"
                        disabled={!canDownloadPdf || record.approval_status !== "checked"}
                      />
                    </div>
                  </td>
                </tr>
...
function ActionButton({
  ...
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      className={`h-8 px-2 inline-flex items-center gap-1 border rounded-md text-xs font-medium whitespace-nowrap ${tones[tone]} ${disabled ? "opacity-45 cursor-not-allowed" : ""}`}>
...
function statusTone(status?: ApprovalStatus | null): string {
  if (status === "checked") return "text-emerald-700";
