...
async function listSupabaseTiRecords(filters: ListFilters = {}): Promise<TiRecord[]> {
  const records = await supabaseFetch<TiRecord[]>(
    "ct_ti_records?select=*&order=created_at.desc"
  );
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const records = isSupabaseConfigured
        ? await supabaseFetch<TiRecord[]>("ct_ti_records?select=ti_no&order=created_at.asc")
        : getTiRecords();
      const idx = records.findIndex((r) => r.ti_no === tiNo);
      return {
        prev: idx > 0 ? records[idx - 1].ti_no : null,
        next: idx !== -1 && idx < records.length - 1 ? records[idx + 1].ti_no : null,
      };
    },
    enabled: options?.query?.enabled !== false && !!tiNo,
    retry: false,
